use crate::{
    db::{NflTiebreakerPick, Registration, Season},
    gridiron::{fetch_rosters_for_teams, find_players},
    types::{Data, Error},
};
...
pub async fn pick_tiebreaker_player(
    ...
    if registrations.is_empty() {
        return Ok(
            "Pick a team during the draft first, then choose a tie-breaker player from that roster.".into(),
        );
    }
    ...
    let roster = fetch_rosters_for_teams(&data.espn_api(), &teams).await?;
    let matches = find_players(&roster, player_query);

    match matches.as_slice() {
        [] => Ok(format!(
            "Couldn't find a player matching \"{player_query}\" on your teams. Try a more specific name."
        )),
        [selected] => {
            let conn = data.db.lock().await;
            let season = Season::default_for_guild(&conn, guild_id)?;
