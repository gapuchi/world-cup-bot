...
impl WcProcessedMatch {
    pub fn is_processed(
        ...
        conn.query_row(
            "SELECT 1 FROM wc_processed_matches WHERE season_id = ?1 AND match_id = ?2",
    ...
    pub fn mark(conn: &Connection, season_id: i64, match_id: i64) -> rusqlite::Result<()> {
        conn.execute(
            "
            INSERT OR IGNORE INTO wc_processed_matches (season_id, match_id)
            VALUES (?1, ?2)
            ",
            params![season_id, match_id],
        )?;
    ...
    pub fn unmark(conn: &Connection, season_id: i64, match_id: i64) -> rusqlite::Result<()> {
        conn.execute(
            "DELETE FROM wc_processed_matches WHERE season_id = ?1 AND match_id = ?2",
            params![season_id, match_id],
        )?;
        Ok(())
    }
